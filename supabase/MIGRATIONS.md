...
| 27 | **`migrations/fix_inventory_reversal.sql`** | ✅ | Bug de auditoría: el trigger de consumo automático (`apply_recipe_consumption`) solo descontaba stock en `INSERT` sobre `order_items` — reducir cantidad o quitar un ítem antes de cobrar no revertía el stock. Reescribe la función para manejar `UPDATE` (delta de cantidad) y `DELETE` (reversión completa) además de `INSERT` |
...
| 32 | **`migrations/fix_qr_ordering.sql`** | ✅ | Consolidado de QR-ordering: columnas `order_type`/`delivery_*`/`payment_method` en `orders`, corrige constraint `delivery_status`, y crea policies RLS anon+authenticated en `restaurant_tables`, `orders`, `order_items`, `categories`, `menu_items`. Aplicada antes de que este doc la registrara — retroactivamente documentada. |
| 33 | **`migrations/staff_members_rls.sql`** | ✅ | Habilita RLS en `staff_members` y crea policy `staff_members_admin` (solo `role='admin'` puede gestionar staff). Requiere que `staff_pins_schema.sql` (#14) ya esté aplicada. |
| 34 | **`migrations/portal_staff_rls.sql`** | ✅ | Policy `portal_tables_update` — permite a meseros marcar mesas como `'available'` al cobrar. La policy original `tables_customer_mark_occupied` solo permitía `status='occupied'`, bloqueando el flujo de cierre de mesa. |
| 35 | **`migrations/fix_pin_security.sql`** | ✅ | Rate-limiting en PIN: agrega `failed_attempts`/`locked_until` a `staff_members`, reemplaza `verify_staff_pin()` con versión que bloquea 15 min tras 5 intentos fallidos, elimina `get_role_credentials()` (credenciales van a `.env`, no a la DB), y restringe `order_events` a INSERT-only para staff no-admin. |
